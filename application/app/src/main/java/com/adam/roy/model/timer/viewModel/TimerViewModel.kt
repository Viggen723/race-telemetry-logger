...
class TimerViewModel(application: Application) : AndroidViewModel(application) {
    private val repository = (application as TelemetryApplication).timerRepository
        ...
        try
        {
            viewModelScope.launch(Dispatchers.IO) {
                ...
                while (isActive) {
                    ...
                    parsedData?.let { data ->
                        timer.update(data.speed, data.timeStamp)
                        updateUI(data)
                    }
                }
    ...
    private fun updateUI(data: Telemetry) {
        ...
        if (currentState == State.RUNNING) {
            ...
            val elapsed = (data.timeStamp - startTime) / 1000.0
